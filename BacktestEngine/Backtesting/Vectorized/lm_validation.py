...
class LmValidation:
    def __init__(self, slm, start='2016-7-1', end='2016-10-1', symbol='ag', data_dir=r'../../Output',
                 valid_dir=r'../../Validation', min_order=2, max_order=8, offsets_average=False, n_offsets=5, tca=None,
                 price='LastPrice', fixed_cost=0.00052, **kwargs):
        '''
    ...
    def run(self):
        ...
        for filename in filenames:
            ...
            data = data[(pd.to_datetime(data.index) >= pd.to_datetime(self._start)) & (
                        pd.to_datetime(data.index) < pd.to_datetime(self._end))]
            if len(data) == 0:
                continue
            else:
                signals = [SLMStrategy(data, self._slm, m - 1, px_th=self._price_threshold).generatingsignal() for m in
                           np.arange(self._min_order, self._max_order + 1)]
                # tcas = [self._tca for m in np.arange(self._min_order + 1, self._max_order + 1)]
                tcas = [self._tca] * (self._max_order - self._min_order + 1)
                validator_ensemble = ensembler(vectorizedbacktest, signals, price=self._price, tcas=tcas,
                ...
                if self._offsets_average:
                    if not self._average_return:
                        self._average_return = [df['strategy'].reset_index(drop=True) for df in
                        ...
                        self._average_return = [df1.add(df2['strategy'].reset_index(drop=True), fill_value=0) for
                                                (df1, df2) in zip(self._average_return, validator_ensemble.results)]
