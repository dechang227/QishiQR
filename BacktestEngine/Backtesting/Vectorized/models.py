...
class LmValidation:
    def __init__(self, slm, symbol='ag', data_dir=r'../../Output', valid_dir=r'../../Validation', max_order=8,
                 offsets_average=False, n_offsets=5):
        '''
    ...
    def run(self):
        ...
        for filename in filenames:
            ...
            if len(data) == 0:
                ...
                signals = [SLMStrategy(data, self._slm, m).generatingsignal() for m in
                           np.arange(1, self._max_order + 1)]
                ...
                if self._offsets_average:
                    if not self._average_return:
                        ...
                        self._average_return = [df1.add(df2['strategy'], fill_value=0) for (df1, df2) in
                                                zip(self._average_return, validator_ensemble.results)]
                        average_performance = average_performance.add(performance, fill_value=0)
                        average_benchmark = average_benchmark.add(validator_ensemble.results[0]['LastPrice'],
                                                                  fill_value=0)
        '''
...
class MajorSeries_MaxPCT:
    ...
    def build(self, model_order=7, freq='5min', start='20161001', end='20161221', offset=0, tca=None, labels=None):
        ...
        all_signals = pd.concat([df[['signal', 'max_pct']] for df in self.signals], axis=0,
...
class MajorSeries_MajorVoting:
    ...
    def compile_signal(self, data, slm, model_orders):
        ...
        signals = [SLMStrategy(data, slm, m, px_th=self._price_threshold).generatingsignal() for m in
                   range(1, model_orders + 1)]
        return signals
...
class OneContractTest:
    ...
    def compile_data(self, commodity, exp_date, offset=0, freq='5min', start='20160701', end='20161031'):
        ...
        tick_day = df_reader(instrument + '*', topdir=self._DATA_DIR + commodity + '/day', offset=offset, freq=freq,
                             day=True, symbol=commodity).get_tick(raw=False)
        tick_night = df_reader(instrument + '*', topdir=self._DATA_DIR + commodity + '/night', offset=offset, freq=freq,
    ...
    def build(self, commodity, exp_date, model_order=7, freq='5min', offset=0, start='20160701', end='20161031',
              tca=None):
...
class NoisyOneContractTest(OneContractTest):
    ...
    def compile_data(self, commodity, exp_date, offset=0, freq='5min', start='20160701', end='20161031'):
        ...
        tick_all = pd.concat([tick_day, tick_night])
        tick_all.sort_index(inplace=True)
        tick_all['LastPrice'] = tick_all['LastPrice'].map(
            lambda x: x * (1 + np.random.uniform(-self.noise, self.noise)))
        return tick_all[(tick_all.index >= start) & (tick_all.index < end)]
...
class MultiContractTest:
    ...
    def compile_data(self, commodity, exp_list, offset=0, freq='5min', start='20160701', end='20161031'):
        ...
        def get_instrument(instrument):
            tick_day = df_reader(instrument + '*', topdir=self._DATA_DIR + commodity + '/day', offset=offset, freq=freq,
                                 day=True, symbol=commodity).get_tick(raw=False)
            tick_night = df_reader(instrument + '*', topdir=self._DATA_DIR + commodity + '/night', offset=offset,
                                   freq=freq, day=False, symbol=commodity).get_tick(raw=False)
            tick_all = pd.concat([tick_day, tick_night])
            tick_all.sort_index(inplace=True)
            return tick_all[(tick_all.index >= start) & (tick_all.index < end)]
    ...
    def build(self, commodity, exp_date, model_order=4, freq='5min', offset=0, start='20160701', end='20161031',
              tca=None):
        self.test_data = self.compile_data(commodity, exp_date, start=start, end=end)
...
class MultiFrequencyTest:
    ...
    def compile_data(self, commodity, exp_list='1705', offset=0, freq=['{}min'.format(i) for i in range(1, 5)],
                     start='20160701', end='20161031'):
    ...
    def build(self, commodity, exp_date, model_order=4, freq=['{}min' for i in range(1, 5)], offset=0, start='20160701',
              end='20161031', tca=None):
