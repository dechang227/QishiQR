...
class SLMStrategy:
    ...
    def generatingsignal(self):
        ...
        if self._price_threshold >= 1:
            ...
            self.data['Direction'] = self.data[self.price].pct_change().apply(
                lambda x: 2 if x > self._price_threshold else (1 if x < -self._price_threshold else 0))
        ...
        #                      how='left')
        self.data = self.data.reset_index().merge(self.slm[['prior', 'max_pct', 'signal']], left_on=['prior'],
                                                  right_on=['prior'],
...
class SLM:
    ...
    def run(self):
        ...
        if self._type == 1:
            ...
            self._slm['signal'] = self._slm[['max', 'max_pct']].apply(
                lambda x: x['max'] if x['max_pct'] > self._threshold else 0, axis=1).astype(int)
        elif self._type == 2:
            # type 2: threshold for the difference of top 2 probs
            self._slm['signal'] = self._slm.apply(lambda x: x['max'] if x['threshold'] > self._threshold else 0,
                                                  axis=1).astype(int)
        elif self._type == 3:
            # type 3: consider only threshold for case of 1 and 2 are top 2 probs
            self._slm['signal'] = self._slm.apply(
                lambda x: 0 if (x['threshold'] <= self._threshold and x['min'] == '0') else x['max'], axis=1).astype(
                int)
        else:
