...
class vectorizedbacktest:
    ...
    def runtest(self):
        ...
        self.result['correct'] = [direct == signal for direct, signal in
                                  zip(self.result['Direction'], self.result['signal'])]
        if self.window > 0:
            ...
            for index in range(self.window, total_rows):
                next_signal = 0 if (self.result['correct_pct'][index] > self.pct_th) else (
                    1 if (self.result['signal'][index] == 2) else (
                        -1 if (self.result['signal'][index] == 1) else signal_bar[-1]))
        ...
        if (self.tca == 'Spread' or self.tca == 'Compound'):
            #
            self.result['strategy'] = self.result['strategy'] - 0.5 * np.log(
                self.result['AskPrice1'] / self.result['BidPrice1']) * self.result['signal_chg_size']
        #  
