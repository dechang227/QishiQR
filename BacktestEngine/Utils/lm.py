...
class LM_model:
    ...
    def t_tests(self, df_train, df_valid, num):
        ...
        lm_stats['t_score_train'] = (lm_stats['max_train_mean'] - lm_stats['max_train']) * np.sqrt(num) / lm_stats[
            'max_train_std']

        # 2. Welch t-test between train and valid sets.
        lm_stats['dof'] = (lm_stats['max_train_std'] ** 2 / num + lm_stats['max_valid_std'] ** 2 / num) ** 2 / \
                          ((lm_stats['max_train_std'] ** 2 / num) ** 2 / (num - 1) + (
                                      lm_stats['max_valid_std'] ** 2 / num) ** 2 / (num - 1))

        lm_stats['t_score'] = (lm_stats['max_train_mean'] - lm_stats['max_valid_mean']) / np.sqrt(
            lm_stats['max_train_std'] ** 2 / num \
...
def gen_strategy(commodity, exp_list, m, n, flg='train', freq=5, interval=1, data_root_dir=r'../../Data',
                 output=r'../../Output'):
    strategy = None
