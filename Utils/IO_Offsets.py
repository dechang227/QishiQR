...
class df_reader:
    ...
    def __init__(self, filepat, topdir, freq='30S', day=True, symbol='rb', offset=[0.1]):
    ...
    def f_fill(self, filename):
        ...
        then return dict of dataframes given offset, and at selected freq.'''
        ...
        def Extract_df_offset(df, offset):
            
            # add offset in minutes
            if self._day:  # day time
                start = pd.to_datetime(dates[0] + ' 09:00:00.0') + timedelta(minutes=offset)
                index1 = pd.date_range(start, dates[0]+' 10:15:00.0', freq=self._freq)
                start_mid = pd.to_datetime(dates[0]+' 10:30:00.0') + timedelta(minutes=offset)
                index2 = pd.date_range(start_mid, dates[0]+' 11:30:00.0', freq=self._freq)
                start_aft = pd.to_datetime(dates[0] + ' 13:30:00.0') + timedelta(minutes=offset)
